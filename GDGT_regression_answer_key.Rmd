...
Answer: The highest adjusted $r^2$ value is found for Winter temperatures at with depth integration to ~150-200 m. (Incidentally, evaluating the model on the basis of any of the other regression statistic yields a simillar result).
...
Answer: Since the strongest correlation is observed between $TEX_{86}^L$ and the Winter temperature integrated over large depth, it appears that *Thaumarchareota* growth and GDGT production occurs throughout the water column, perhaps especially in winter. 
...
Answer: The temperature estimate is systematically higher with the local calibration. The variation in the record is also dampened. 

...
Once you've chosen the variables you would linke to regress, use `lm()` to calculate a linear regression. `summary()` is a useful function for printing an overview of the model. 
```{r}
...
To compare the regressions for each temperature profile, plot the adjusted $r^2$ value of each regression vs the integrated depth, using color to denote season. 
...
3. What does this suggest about GDGT production and preservation at these sites?
...
```{r warning = FALSE}
data <- data %>% mutate(
  kim2010 = (67.5 * TEX86L + 46.9), kim2012 = (50.8 * TEX86L + 36.1), best_model = (27.947 * TEX86L + 22.742)
) %>% 
